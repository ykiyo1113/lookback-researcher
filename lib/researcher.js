...
// ─── Wantedly 検索結果タイトルから担当者を解析 ──────────────────────
function parseWantedlyTitle(title, snippet, url) {
  if (!/wantedly\.com\/(id\/|profiles\/)/.test(url)) return null;

  // "田中 花子のプロフィール - Wantedly" / "田中花子 | Wantedly"
  const cleaned = title
    .replace(/のプロフィール.*/,  '')
  ...
  const isJa = /[一-龯ぁ-んァ-ン]/.test(cleaned);
  const isEn = /^[A-Z][a-z]/.test(cleaned);
  if (!isJa && !isEn) return null;

  const role = snippet.match(
    /(?:マーケティング|広報|PR|ブランド|宣伝|代表取締役|取締役|社長|Manager|Director)[^\s。、\n]{0,20}/
  )?.[0] || '';
...
async function researchCompany(companyName, emit) {
  let domain = null;
  const collected = { wantedly: [], hp: [] };

  // ① Wantedly（Yahoo/Brave検索 → タイトルから名前抽出）+ ドメイン取得
  emit({ type: 'status', source: 'wantedly', message: '検索中...' });
  try {
    const query = `"${companyName}" wantedly.com 広報 OR マーケティング OR 代表取締役`;
    const items = await webSearch(query);

    domain = extractDomainFromSearchResults(items);
    if (domain) emit({ type: 'domain', domain });

    for (const { title, url, snippet } of items) {
      const person = parseWantedlyTitle(title, snippet, url);
      if (person) collected.wantedly.push(person);
    }
    console.log(`  🔍 Wantedly: ${collected.wantedly.length}名`);
    emit({ type: 'source_result', source: 'Wantedly', persons: collected.wantedly });
  } catch (e) {
    console.log(`  ⚠️ Wantedly検索エラー: ${e.message}`);
