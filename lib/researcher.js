...
async function extractFromCompanyHP(domain, emit) {
  ...
  for (const tryUrl of [baseUrl, `https://www.${domain}`]) {
    try {
      ...
      visited.add('/');
      const homePersons = extractPersonsFromText([$.text()]);
      addPersons(homePersons, tryUrl);
      emit({ type: 'log', message: `/ → ${homePersons.length}名, テキスト${$.text().trim().length}文字` });
  ...
  for (const path of sorted) {
    ...
    try {
      ...
      addPersons(found, pageUrl);
      emit({ type: 'log', message: `${path} → ${found.length}名, テキスト${$.text().trim().length}文字` });
  ...
  for (const path of level2) {
    ...
    try {
      ...
      const found = extractPersonsFromText([$.text()]);
      addPersons(found, pageUrl);
      emit({ type: 'log', message: `${path} → ${found.length}名` });
    } catch {
      emit({ type: 'log', message: `${path} → アクセス失敗` });
    }
  }
