...
// ─── ドメイン: Yahoo検索「公式サイト」から取得（最も信頼性が高い） ────
async function findCompanyDomain(companyName) {
  try {
    const items = await yahooSearch(`"${companyName}" 公式サイト`);
    // 「公式」タイトルの結果を優先
    for (const { url, title } of items) {
      if (!url || !title) continue;
      if (!/公式|Official/i.test(title)) continue;
      try {
        const host = new URL(url).hostname.replace(/^www\./, '');
        if (!DOMAIN_BLACKLIST.some(b => host.includes(b))) return host;
      } catch {}
    }
    // 公式タイトルがなければ最初の非ブラックリストURLを使用
...
async function researchCompany(companyName, emit) {
  ...
  try {
    ...
    // PR TIMESのURL記載 → なければYahoo「公式サイト」検索で取得
    domain = extractCompanyUrl(bodyText) || await findCompanyDomain(companyName);
