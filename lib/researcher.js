...
// ─── ドメイン選定 ────────────────────────────────────────────────────
function selectBestDomain(hosts) {
  const cleaned = (hosts || [])
    .map(h => h.replace(/^www\./, ''))
    .filter(h => !DOMAIN_BLACKLIST.some(b => h.includes(b)));
  return cleaned.find(h => h.endsWith('.co.jp'))
      || cleaned.find(h => h.endsWith('.com') && h.split('.').length === 2)
      || cleaned[0]
      || null;
}
...
async function researchCompany(companyName, emit) {
  ...
  try {
    ...
    domain = extractCompanyUrl(bodyText) || selectBestDomain(allExternalHosts);
